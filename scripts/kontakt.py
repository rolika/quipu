...
class Kontakt(Csomo):
    ...
    def listanezet(self):
        szemely = Csomo.kon.szemely.select("szemely", azonosito=self.szemely).fetchone()
        szemely = Szemely(**szemely)
        szervezet = Csomo.kon.szervezet.select("szervezet", azonosito=self.szervezet).fetchone()
